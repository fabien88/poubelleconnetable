...
public class TrashServiceTest {
	...
	private final LocalServiceTestHelper helper = new LocalServiceTestHelper(
			new LocalDatastoreServiceTestConfig()
					.setNoStorage(true)).setEnvAuthDomain("localhost");
	...
	public void getTrashLogsFor() throws Exception {
		emptyTrash(); // This create and empty trashes to feed log in datastore

		// Check log empty trash has been made 1 second ago maximum
		Assert.assertEquals(
				"trash log date is incorrect",
				new Date().getTime(),
		...
		// And that use has a single empty log for trash2
		ReflectionAssert.assertReflectionEquals(
	...
	public void deleteTrash() throws Exception {
		...
		CustomReflectionAssert.reflectionNotContains(new Trash("notexistingtrash", 2D), trashService.getTrashes(
				YET_ANOTHER_USER));
